...
$$
\min\_{h, z} f(z;h)\qquad s.t. \quad z = \operatorname{argmax}\_{\theta_h} f(\theta_h;h).
$$
...
$$
\min\_h f(h)\le \min\_\alpha \mathbb E\_{h\sim p\_{\alpha}(h)}[f(h)],
$$

where $p(h|\alpha)$ can be parametrized by a sequential network, of which the gradient becomes tractable:
$$
\nabla\_\alpha \mathbb E\_{p_\alpha(h)}[f(h)] = \mathbb E\_{p\_\alpha (h)}[f(h)\nabla\_\alpha \log {p\_\alpha}(h)].
$$
...
$$
\nabla\_\alpha \mathbb E\_{p_\alpha(h)}[f(h)]\approx\nabla\_\alpha f(\sigma(\alpha))
$$
...
$$
\nabla\_\alpha \mathbb E\_{p_\alpha(h)}[f(h)]\approx \mathbb E\_{p(u)}\nabla\_\alpha f(\sigma(z/t));\quad z:=\log\frac{\alpha}{1-\alpha} + \log\frac{u}{1-u};\quad u\sim\operatorname{Uniform}(0, 1).
$$
